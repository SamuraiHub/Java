...
public class SetRangeSum {
    ...
    void erase(int x) {
        ...
        if (right != null && right.key == x) {
            ...
            {
                if(right.parent.left.key == right.key) {right.parent.left = null; }
                else { right.parent.right = null; }
            }
            
            if(right.left != null) { right.left.parent = null; } 
            if(right.right != null) { right.right.parent = null; }
    ...
    void solve() throws IOException {
        ...
        for (; i < n; i++) {
            ...
            switch (type) {
                case '+' : {
                    int x = nextInt();
                    insert(((x % MODULO) + last_sum_result) % MODULO);
                } break;
                case '-' : {
                    int x = nextInt();
                    erase(((x % MODULO) + last_sum_result) % MODULO);
                } break;
                case '?' : {
                    int x = nextInt();
                    String rs = find(((x % MODULO) + last_sum_result) % MODULO) ? "Found" : "Not found";
                    out.println(rs);
                } break;
                case 's' : {
                    int l = nextInt();
                    int r = nextInt();
                    long res = sum(((l % MODULO) + last_sum_result) % MODULO, ((r % MODULO) + last_sum_result) % MODULO);
                    out.println(res);
...
//select the operator                
            ...
            int opi = operatorRand.nextInt(operators.length);

//select a random number to be used                
            Random randNumber = new Random();
            int num = randNumber.nextInt(maxNumber);

            if (opi != 3) {
                inputData = inputData + operators[opi] + " " + num + "\n";
            } else {
                Random randNumber1 = new Random();
                int a = randNumber1.nextInt(maxNumber);
                Random randNumber2 = new Random();
                int b = randNumber2.nextInt(maxNumber);
                inputData = inputData + operators[opi] + " " + a + " " + b + "\n";
            }
        }
