...
public class SuffixTreeFromArray {
    ...
    public class Edge {
        ...
        {
          return  this.node+", "+this.start+", "+this.end;
        }
    }
    ...
    Map<Integer, List<Edge>> SuffixTreeFromSuffixArray(
        ...
        List<Edge> edges = new ArrayList<Edge>(text.length() >> 1);
        edges.add(new Edge(1,suffixArray[0],suffixArray.length));
        tree.put(0, edges);
        int n = 1;
        int nd = 0;
        ...
        for(int i = 1; i < suffixArray.length; i++)
        {
            ...
            {
               n++; 
               tree.get(0).add(new Edge(n,suffixArray[i], suffixArray.length));
               nd = 0;
            }
            else if(lcpArray[i-1] == lcpArray[i-2])
            {
                n++;
                tree.get(nd).add(new Edge(n,suffixArray[i]+lcpArray[i-1],suffixArray.length));
            }
            else
            {
                int df = lcpArray[i-1];
                ...
                if(df > lcpArray[i-2])
                {
                  e = tree.get(nd).get(tree.get(nd).size()-1);
                  nl = e.end - e.start;
                  df = df-lcpArray[i-2];
                }
                else
                {
                   e = tree.get(0).get(tree.get(0).size()-1);
                   nl = e.end - e.start;
                   while(nl < df)
                   {
                     df = df - nl;
                     e = tree.get(e.node).get(tree.get(e.node).size()-1);
                     nl = e.end - e.start;
                   }
                }
                ...
                if(nl >  df)
                {
                   ...
                   if(tree.get(e.node) != null)
                   {
                       int node = e.node;
                       n++;
                       e.node = n;
                       edges.add(new Edge(node,e.end,en));
                       n++;
                       edges.add(new Edge(n,suffixArray[i]+lcpArray[i-1],suffixArray.length));
                       tree.put(e.node,edges);
                       nd = e.node;
                   }
                   else
                   {
                     n++;
                     edges.add(new Edge(n,e.end,en));
                     n++;
                     edges.add(new Edge(n,suffixArray[i]+lcpArray[i-1],suffixArray.length));
                     tree.put(e.node,edges);
                     nd = e.node;
                ...
                { 
                  n++;
                  tree.get(e.node).add(new Edge(n,suffixArray[i]+lcpArray[i-1],suffixArray.length));
                  nd = e.node;
                }
            } 
        }
