...
public class UnrolledLinkedList<E> extends AbstractList<E> implements List<E>, Serializable 
{
     ...
     * @throws IllegalArgumentException if <tt>nodeCapacity</tt> is less than 10
     */
    public UnrolledLinkedList(int nodeCapacity) throws IllegalArgumentException {

        if (nodeCapacity < 10) {
            throw new IllegalArgumentException("nodeCapacity < 10");
        }
    ...
    private void insertIntoNode(Node node, int ptr, E element) {
        ...
        if (node.numElements == nodeCapacity) {
            ...
            int elementsToMove = nodeCapacity / 2; //15/2 = 7.5 7
            int startIndex = nodeCapacity - elementsToMove; // 8
            int i;

            System.arraycopy(node.elements, startIndex, newNode.elements, 0, elementsToMove);
            Arrays.fill(node.elements, startIndex, nodeCapacity, null);
            
            node.numElements -= elementsToMove; //8
            newNode.numElements = elementsToMove; //7
    ...
    private void removeFromNode(Node node, int ptr) {
        ...
        System.arraycopy(node.elements, ptr+1, node.elements, ptr, node.numElements-ptr);
    ...
    private void mergeWithNextNode(Node node) {
        ...
        System.arraycopy(next.elements, 0, node.elements, node.numElements, next.numElements);
        
        Arrays.fill(next.elements, 0, next.numElements, null);
