...
public class DistPreprocessLarge {
    private static class Impl {
        ...
        long shortcut(int v, ArrayList<Integer> scs) {
            ...
            Long importance = (shortcuts << 1) - adj[0][v].size() - adj[1][v].size();
            return importance;
        }
        ...
        void preprocess() {
            ...
            {
                sc[i] = new ArrayList<>();
                q[i] = new Entry(shortcut(i,sc[i]), i);
            }
            
             Arrays.parallelSort(q,0,pos);
    ...
    private long distance(ArrayList<Integer>[] adj1, ArrayList<Long>[] cost1, int s, int t, int v, long dist) 
    {
        ...
        while (position > 1) 
        {
            ...
            if(e.cost >= dist)
                return INFINITY;
            int in = adj[1][t].indexOf(e.node);
            if(in != -1)
            {
              mn++;  
              if(cost[1][t].get(in)+e.cost < dist)
                  return cost[1][t].get(in)+e.cost;
              if(adj[1][t].size()-1 == mn)
                  return INFINITY;
            }
            ...
            {
                int n = adj1[e.node].get(V);
                if(n == v || level[n] < level[v])
                    continue;
                ...
                if (dst1[n] > 0) 
                {
                    if(pd < mF[dst1[n]].cost) 
                    {
                        ...
                        mF[pos] = new Entry(pd, n);
                        dst1[mF[pos].node] = pos;
                    }
                } 
                else if(dst1[n] == -1)
                {
                    insert(mF, new Entry(pd, n), dst1, position);
                    position++;
                }
            }
        }
