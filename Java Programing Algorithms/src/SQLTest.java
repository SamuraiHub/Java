...
public class SQLTest {
   ...
   public void creerTable(Connection con) throws SQLException{
       ...
       try {
           ...
           st.execute("drop table if exists CREDENTIAL;");
   ...
   public static String byteToBase64(byte[] data){
    ...
    KeySpec spec = new PBEKeySpec("Mmammaj1#!".toCharArray(), "!AKf3eG5mT#".getBytes(), 1024, 256);
    ...
    byte[] ciphertext = cipher.doFinal("Hey Ya Nub! How u doin?!".getBytes("UTF-8"));
    String ct = new String(ciphertext, "ISO8859-2");
    ...
    String[] ocs = new String[3]; int i = 0;
    ResultSet rs = stat.executeQuery("select * from people;");
    while (rs.next()) {
      System.out.println("name = " + rs.getString("name"));
      System.out.println("job = " + (ocs[i++] = rs.getString("occupation")));
    }
    
  cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
 cipher.init(Cipher.DECRYPT_MODE, secret, new IvParameterSpec(iv));
 String plaintext = new String(cipher.doFinal(ocs[2].getBytes("ISO8859-2")), "UTF-8");
 System.out.println(plaintext);

    rs.close();
    conn.close(); */
        SQLTest up = new SQLTest();
        Class.forName("org.sqlite.JDBC");
        Connection conn =
      DriverManager.getConnection("jdbc:sqlite:test.db");
        up.creerTable(conn);
        System.out.println(up.createUser(conn, "TheGreatSamurai", "MSauammn"));
        System.out.println(up.authenticate(conn, "TheGreatSamurai", "MSauammn"));
        Statement stat = conn.createStatement();
        ResultSet rs = stat.executeQuery("select * from CREDENTIAL;");
        while (rs.next()) {
