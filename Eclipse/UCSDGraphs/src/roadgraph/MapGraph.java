...
public class MapGraph {
			 					     ...
			 					     GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
        ...
        if(s == t)
        	return new ArrayList<GeographicPoint>(Arrays.asList(new GeographicPoint[]{start}));
        
        while (!r.isEmpty()) 
        {
            int e = r.remove();
            nodeSearched.accept(GPL.get(e));
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		return shotestPath(GPS.get(start),GPS.get(goal),false,nodeSearched);
	}
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		return shotestPath(GPS.get(start),GPS.get(goal),true,nodeSearched);
	}
	
	/** Modifies the mini heap (used when a new node was inserted or its distance modified) for shortest distance calculation 
	 * 
	...
	public void Modify(List<Ride_Distance> rds, int dst[], int vertex)
    {
        int pos = dst[vertex];
	...
	/** finds the list of vertices (intersections) on the shortest path form start to destination
	 * 
	 * @param s start vertex
	 * @param t  end vertex
	 * @param aStar a boolean indicating with A* algorithm is used in search or not 
	 * @param nodeSearched A hook for visualization. 
	 * @return the list of vertices (intersections) on the shortest path form start to destination
	 */
	private List<GeographicPoint> shotestPath(int s, int t, boolean aStar, 
			Consumer<GeographicPoint> nodeSearched) 
	{
		List<Ride_Distance> rds = new ArrayList<>(mgaj.getNumVertices()+1);
		int[] b = new int[mgaj.getNumVertices()];
        rds.add(null);
        int dst[] = new int[mgaj.getNumVertices()];
        Arrays.fill(dst, -1);
        dst[s] = 1;
        ...
        while (position > 1) 
        {
            ...
            position--;
            nodeSearched.accept(GPL.get(rd.vertex));
            count++;
            ...
            if(rd.vertex == t)
            {
            	...
            	nodeSearched.accept(GPL.get(t));
            	List<GeographicPoint> path = new LinkedList<>();
                ((LinkedList<GeographicPoint>) path).addFirst(GPL.get(t));
                int e = b[t];
                while(e != -1)
                {
                	((LinkedList<GeographicPoint>) path).addFirst(GPL.get(e));
            ...
            List<Integer> neighbors = mgaj.getNeighbors(rd.vertex);
            List<Double> dtns =  mgaj.getDistanceToNeighbors(rd.vertex);
                       
            for (int i  = 0; i < neighbors.size(); i++) 
            {
                int avr = neighbors.get(i);
                double pd = rd.distance+dtns.get(i)+ (aStar ? GPL.get(avr).distance(GPL.get(t)) : 0);   
