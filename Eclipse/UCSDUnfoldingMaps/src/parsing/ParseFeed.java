...
public class ParseFeed {
	...
	public static List<PointFeature> parseEarthquake(PApplet p, String fileName) {
		...
		processing.data.XML rss = p.loadXML(fileName);
		// Get all items
		processing.data.XML[] itemXML = rss.getChildren("entry");
		PointFeature point;
		...
		for (int i = 0; i < itemXML.length; i++) {
				...
				processing.data.XML[] catXML = itemXML[i].getChildren("category");
	 ...
	 */
	private static Location getLocationFromPoint(processing.data.XML itemXML) {
		// set loc to null in case of failure
		Location loc = null;
		processing.data.XML pointXML = itemXML.getChild("georss:point");
	 ...
	 */
	private static String getStringVal(processing.data.XML itemXML, String tagName) {
		// Sets title if existing
		String str = null;
		processing.data.XML strXML = itemXML.getChild(tagName);
	 ...
	 */
	private static float getFloatVal(processing.data.XML itemXML, String tagName) {
